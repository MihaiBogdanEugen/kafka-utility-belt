    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (res.getString("config") == null && !(res.getString("security_protocol").equals("PLAINTEXT"))) {
                LOGGER.error("config is required for all protocols except PLAINTEXT");
                success = false;
            } else {
                ...
                if (res.getString("bootstrap_servers") != null) {
                    workerProps.put(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG, res.getString("bootstrap_servers"));
                } else {
                    ...
                    Map<String, String> endpoints = ClusterStatus.getKafkaEndpointFromZookeeper(zkConnectString, res.getInt("timeout"));

                    String bootstrapBroker = endpoints.get(res.getString("security_protocol"));
                    if (bootstrapBroker == null) {
                        throw new RuntimeException("No endpoints found for security protocol [" + res.getString("security_protocol") + "]. Endpoints found in ZK [" + endpoints + "]");
                    }
            ...
            LOGGER.error("Error while running kafka-ready.", e);
            success = false;
        }
